...
orderService.get = (orderId) => {
    return db.one('SELECT * FROM Customer WHERE orderId = ${orderId}', {orderId})
}

orderService.put = (productId, customerId, orderDate, orderNum, price, total, color, orderStatus, paymentType) => {
    return db.none('UPDATE Customer SET productId = ${productId}, customerId = ${customerId}, orderDate = ${orderDate}, orderNum = ${orderNum}, price = ${price}, total = ${total}, color = ${color}, orderStatuse = ${orderStatus}, paymentType = ${paymentType} WHERE orderId = ${orderId}', 
     {productId, customerId, orderDate, orderNum, price, total, color, orderStatus, paymentType})
}

orderService.delete = (orderId) => {
    return db.none('DELETE FROM Customer WHERE orderId = ${orderId}', {orderId})
}
